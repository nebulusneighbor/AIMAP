...
def dict_to_midi(token_sequence, output_path):
    """
    Translates a sequence of string tokens ('i-X', 'o-X', 'p-X', 'd-X', 'b-1', 'g-X') 
    ...
    while i < len(token_sequence):
        tok = str(token_sequence[i]) # Ensure it's a string
        
        if tok.startswith("g-"): 
            # Extract genre and normalize
            current_genre = tok.split("-")[1].lower().replace("content_", "")
            if current_genre == "elegance": current_genre = "elegant"
            i += 1
            continue
        ...
        if tok.startswith("o-"):
            ...
            # Remap drum pitches (stored as 128+pitch in some versions)
            if current_inst == 128 and pitch >= 128:
                pitch -= 128
            ...
            # CATEGORICAL VELOCITY LOGIC
            # 1. Non-Elegant Drums: Fixed 100 (to satisfy 'not 80')
            # 2. Elegant Drums & All Other Instruments: Randomized 15-95
            if current_inst == 128 and current_genre != "elegant":
                vel = 100
            else:
                vel = random.randint(15, 95)
